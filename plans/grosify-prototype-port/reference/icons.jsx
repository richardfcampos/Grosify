/* Grosify — one consistent line-icon family (stroke, 24px, round).
   Source of truth for the icon set used across every prototype screen.
   Port target: apps/web/src/features/ui/icon.tsx (typed React component). */
const PATHS = {
...
/* Other prototype primitives to port (see SCREENS.md for which screens use them):
   - Money({cents,size,tone})  -> wraps @grosify/ui MoneyValue, Math.abs(cents)
   - Sparkline({data,w,h,color}) -> svg polyline; up=red down=green (brand: money colors only)
   - Sheet({open,onClose,children,dark}) -> bottom sheet modal (CheckSheet host)
   - Empty({icon,title,body,action}) -> dashed-circle empty state
   - CatIcon({cat}) -> neutral grey category dot (NO decorative color — brand rule) */

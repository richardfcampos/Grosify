...
| 7 | i18n sweep (all 6 locales), QA against DESIGN.md/brand rules, cleanup | ◑ — QA visual ao vivo via /browse (dev :5176, conta de teste): design system renderiza fiel (Lexend/Anton/Plex Mono carregam, tokens `.gro-app`, cards, bottom nav). Telas verificadas: Auth, Home, Items, Lists, lista-detail, Compra escura, ItemForm. **Achado+corrigido**: `.gro-field width:100%` esmagava inputs de qty (w-16) → largura inline. Classes reutilizáveis `.gro-field`/`.gro-sheet-*` no theme.css. **Sheets tokenizados** (typecheck+build verdes): CheckItemSheet/QuickAddSheet/UnknownBarcodeSheet/BrandPicker migrados de stone/zinc hardcoded → `.gro-sheet-*`/`.gro-field`/`Button`/tokens, herdando o cascade (auto-dark no Modo Compra). Bug corrigido: UnknownBarcodeSheet renderizava **branco** dentro da Compra escura. CheckItemSheet: botão confirmar agora `Button` primary (verde economia, conforme referência) em vez de amarelo hardcoded; BrandPicker perdeu a prop `dark` (adapta pelo contexto). **Pendente**: sweep final i18n (6 locais), PreçoDetail tabela-lojas dedicada, QA visual ao vivo dos sheets reskinados. |

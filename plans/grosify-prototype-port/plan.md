...
| 6 | Inventário + ItemForm + Casa(membros/lojas) + Histórico + Auth + Onboarding visual pass | ◑ (typecheck+build green) — feito: **Inventário** (seg filtros, rows `card row-sep`, Badge status **neutral** por DESIGN — desvio consciente do protótipo que usa subiu/oferta, qty Anton "em casa"); **Histórico** (date-block Anton dia + mono mês, loja, total mono + PriceChange(-economia) por sessão, footer "Economia acumulada" MoneyValue — dado real estimado−pago; nova chave `history.accumulatedSavings`×6); **Casa** = membros (avatar inicial, Badge papel neutral, mantém select/remover) + lojas (Icon store + chev, `card row-sep`). **Pendente**: ItemForm, Auth, Onboarding visual. |

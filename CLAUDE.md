...
- Dinheiro sempre em unidades mínimas da moeda (integer) — `formatCurrency`/`parseToMinorUnits` de `@grosify/shared`. Moeda é por household (`membership.currency`, ISO 4217 via Intl nativo — sem lib de moedas). Nunca assumir 2 casas decimais (JPY=0, BHD=3).
- Listas de compras são múltiplas por casa (`shopping_lists`), cada uma `isRecurring` ou avulsa.

...
| 2026-07-06 | **NFC-e import** — QR-only via SEFAZ; parsers próprios SVRS/SP/MG (grátis) + SE via Infosimples (env-gated, ~R$100/mês); matching hybrid fuzzy + Gemini embedding (env-gated); quota Free 2/mês Pro 60/mês; cache de embedding por item; CPF descartado antes de persistir (LGPD); specs em `.specs/features/nfce-import/`; checklist operacional em `docs/setup-checklist-operacional.md` | Killer feature Pro decisão do usuário; permite importar nota fiscal (múltiplos itens+preços+loja em 1 clique) |
| 2026-07-06 | **NL-List (geração por texto)** — Gemini structured output (geração de lista via prompt natural); entrada dupla (criação de lista nova + adicionar-por-texto em lista existente); Pro-only SEM degustação; revisão antes de criar (reutiliza linha do nfce + matching); rota stateless `/ai/generate-list` rate-limited 10/min + 1 retry; env-gated `GEMINI_API_KEY` (501 sem chave); specs em `.specs/features/nl-list/` | Segunda aposta IA fase 8 roadmap; entrada natural de lista (não precisa nome exato ou categorizar antes); reusa matching+linha do nfce; decisão Pro-only porque custeia chamadas Gemini |

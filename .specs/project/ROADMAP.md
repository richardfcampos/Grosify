...
| 6 | Polish + lançamento público | Tela Ajustes, LGPD (export JSON + excluir conta/casa), seed de itens comuns pt-BR, alertas in-app de preço (já em PrecoSheet/CheckItemSheet) | ✅ Pronto e verificado. **Bug de segurança corrigido**: vazamento de dados entre contas no Dexie local (initHousehold limpa cache na troca). Falta: privacy policy escrita (texto), compartilhar recibo WhatsApp |

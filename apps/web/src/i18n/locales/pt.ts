export default {
  translation: {
    appearance: {
      title: 'Aparência',
      theme: 'Tema',
      light: 'Claro',
      dark: 'Escuro',
      system: 'Sistema',
      direction: 'Direção visual',
      dir: {
        painel: 'Painel',
        painelTag: 'Gestor de dinheiro — calmo, dados à frente.',
        mercado: 'Mercado',
        mercadoTag: 'Vernáculo de encarte — dinheiro gigante, etiqueta e carimbo.',
        recibo: 'Recibo',
        reciboTag: 'Registro auditável — mono tabular, linhas finas, sobriedade.',
      },
    },
    auth: {
      networkError: 'Sem conexão com o servidor. Tente de novo.',
      loginTitle: 'Entrar no Grosify',
      subtitle: 'Sua lista, estoque e preços, em qualquer aparelho.',
      signupTitle: 'Criar conta',
      email: 'E-mail',
      password: 'Senha',
      passwordHint: 'Senha (mínimo 8 caracteres)',
      name: 'Seu nome',
      login: 'Entrar',
      loggingIn: 'Entrando…',
      signup: 'Criar conta',
      signingUp: 'Criando…',
      noAccount: 'Não tem conta?',
      hasAccount: 'Já tem conta?',
      invalidCredentials: 'E-mail ou senha incorretos',
      signupFailed: 'Não foi possível criar a conta',
      forgotPassword: 'Esqueci minha senha',
      forgotTitle: 'Recuperar senha',
      forgotCta: 'Enviar link de recuperação',
      forgotSent: 'Se existir uma conta com esse e-mail, enviamos um link de recuperação. Confira sua caixa de entrada.',
      sending: 'Enviando…',
      backToLogin: 'Voltar para entrar',
      resetTitle: 'Definir nova senha',
      resetCta: 'Salvar nova senha',
      resetInvalid: 'Link inválido ou expirado. Peça um novo.',
      newPassword: 'Nova senha',
      saving: 'Salvando…',
      verifyTitle: 'Verificação de e-mail',
      verifySuccess: 'E-mail verificado! 🎉 Sua conta está protegida.',
      verifyFailed: 'Não foi possível verificar. O link pode ter expirado.',
      verifyBanner: 'Confirme seu e-mail para proteger sua conta.',
      verifyResend: 'Reenviar',
      verifyResent: 'E-mail enviado.',
      goToApp: 'Ir para o app',
      logout: 'Sair',
    },
    household: {
      createTitle: 'Crie sua casa',
      createSubtitle: 'A casa reúne lista, estoque e preços compartilhados com sua família.',
      namePlaceholder: 'Nome da casa (ex.: Casa da Ana)',
      currency: 'Moeda dos preços',
      create: 'Criar casa',
      creating: 'Criando…',
      createFailed: 'Não foi possível criar a casa',
      inviteHint: 'Recebeu um convite? Abra o link que te enviaram.',
      inviteTitle: 'Convite para uma casa',
      inviteText: 'Você foi convidado a entrar numa casa no Grosify com o código <1>{{code}}</1>.',
      join: 'Entrar na casa',
      joining: 'Entrando…',
      inviteEmailLabel: 'Convidar por e-mail',
      inviteEmailPlaceholder: 'e-mail de quem você quer convidar',
      inviteEmailCta: 'Enviar convite',
      inviteEmailSent: 'Convite enviado para {{email}}.',
      inviteByName: '{{inviter}} convidou você',
      inviteToHousehold: 'para entrar na casa "{{household}}"',
      inviteMismatch: 'Este convite é para outro e-mail. Entre com o e-mail convidado para aceitar.',
      yourHouses: 'Suas casas',
      switch: 'Trocar',
      active: 'Ativa',
      newHousehold: 'Criar nova casa',
      switchPending: 'Sincronize as mudanças pendentes antes de trocar de casa.',
    },
    dashboard: {
      planFree: 'Plano Grátis',
      planPro: 'Plano Pro',
      ready: 'Sua casa está pronta 🎉',
      comingSoon: 'Itens, lista e preços chegam nas próximas fases.',
      inviteSection: 'Convidar alguém',
      inviteDescription: 'Quem entrar pelo link compartilha lista, estoque e preços desta casa.',
      generateInvite: 'Gerar link de convite',
      generating: 'Gerando…',
      copyLink: 'Copiar link',
      copied: 'Copiado!',
      language: 'Idioma',
    },
    restock: {
      belowEstimate: 'Abaixo do estimado',
      belowEstimateValue: '{{under}} de {{total}} listas',
      bestPrice: 'Melhor preço pego',
      noLists: 'Crie uma lista recorrente pra gerar a reposição.',
      missingCount: '{{count}} itens faltando',
      savedIn: 'Você economizou em {{month}}',
      noPrice: '{{count}} sem preço',
      estimated: 'estimado',
      title: 'Reposição do mês',
      subtitle: 'Falta comprar, segundo seu estoque',
      dueToday: 'Hoje é dia de comprar',
      nothing: 'Estoque em dia! Nada a repor.',
      noTargets: 'Defina o recomendado/mês nos itens para gerar a reposição automática.',
      toBuy: 'Comprar',
      estimatedTotal: 'Total estimado',
      startShopping: 'Iniciar compra',
      doInventory: 'Fazer inventário',
      invite: 'Convidar alguém',
    },
    forecast: {
      daysLeft_one: 'acaba em ~{{count}}d',
      daysLeft_other: 'acaba em ~{{count}}d',
      teaser: 'Previsão de reposição — Pro',
    },
    nav: {
      home: 'Início',
      shop: 'Comprar',
      lists: 'Listas',
      items: 'Itens',
      stores: 'Lojas',
      prices: 'Preços',
      stock: 'Estoque',
      settings: 'Ajustes',
      planPro: 'Plano Pro',
      planFree: 'Plano Free',
    },
    lists: {
      recommended: 'Recomendado',
      removeConfirm: 'Remover "{{name}}" da lista?',
      deleteListConfirm: 'Excluir a lista "{{name}}" e todos os itens dela?',
      createItem: 'Criar "{{name}}"',
      title: 'Listas',
      newList: 'Nova lista',
      listName: 'Nome da lista',
      listNameHint: 'ex.: Compras do mês, Churrasco',
      recurring: 'Recorrente (todo mês)',
      recurringTag: 'Recorrente',
      oneTimeTag: 'Avulsa',
      private: 'Lista privada (só você vê)',
      privateTag: 'Privada',
      unassigned: 'Sem responsável',
      budget: 'Orçamento',
      budgetHint: 'Limite de gasto (opcional)',
      icon: 'Ícone',
      color: 'Cor',
      dayOfMonth: 'Dia {{day}}',
      recurrenceTag: { weekly: 'Semanal', biweekly: 'Quinzenal', monthly: 'Mensal' },
      noLists: 'Nenhuma lista ainda. Crie a primeira.',
      deleteList: 'Excluir lista',
      empty: 'Nenhum item nesta lista.',
      addItem: 'Adicionar item',
      selectItem: 'Escolha um item',
      qty: 'Qtd',
      estimatedTotal: 'Total estimado',
      missingPrices: '{{count}} sem preço',
      remove: 'Remover',
      inventory: 'Inventário',
      inventoryTitle: 'O que tem em casa',
      onHand: 'Em casa',
      needed: 'Comprar',
      inventoryHint: 'Conte o que já tem; calculamos quanto falta comprar.',
    },
    prices: {
      title: 'Preços',
      intelKicker: 'Inteligência de preço',
      intelSub: 'Onde está mais barato e como variou',
      cheapestStore: 'Mais barato em {{store}}',
      record: 'Registrar preço',
      rating: 'Avaliação',
      history: 'Histórico de preços',
      noPrices: 'Sem preços registrados.',
      cheapest: 'Mais barato',
      cheapestAt: 'Mais barato: {{price}} em {{store}}',
      seenOn: 'em {{date}}',
      avg3m: 'Média 90 dias: {{price}}',
      priceUp: 'Subiu {{delta}} ({{pct}}%) desde {{date}}',
      priceDown: 'Caiu {{delta}} ({{pct}}%) desde {{date}}',
      store: 'Loja',
      price: 'Preço',
      selectStore: 'Escolha a loja',
      searchStore: 'Buscar loja…',
      noStores: 'Cadastre uma loja primeiro.',
    },
    priceInsights: {
      title: 'Compre agora ou espere',
      teaser: 'Compre agora ou espere — Pro',
      verdict: {
        buy: 'Bom momento: {{current}} está abaixo da média de {{avg}}.',
        wait: 'Melhor esperar: {{current}} está acima da média de {{avg}} e subindo.',
        neutral: 'Preço estável perto da média de {{avg}}.',
      },
      swap: '{{cheaper}} está {{pct}}% mais barata que {{pricier}} em {{store}}.',
    },
    brands: {
      label: 'Marca',
      none: 'Sem marca',
      new: 'Nova marca',
      newPlaceholder: 'Nome da marca',
      title: 'Marcas',
      hint: 'Marcas deste item (ex.: Camil, Tio João). Opcional.',
      empty: 'Nenhuma marca cadastrada.',
      delete: 'Excluir marca',
      deleteConfirm: 'Excluir a marca "{{name}}"?',
      forBarcode: 'Marca deste código',
      preferred: 'Marca preferida',
    },
    barcode: {
      unknownTitle: 'Código não cadastrado',
      chooseItem: 'A qual item pertence?',
      searchItem: 'Buscar item…',
      orCreate: 'Ou crie um item novo',
      createItem: 'Criar item',
      saveCode: 'Salvar código',
      change: 'trocar',
      lookingUp: 'Buscando produto…',
      offFound: 'Nome preenchido via OpenFoodFacts',
    },
    catalog: {
      searchPlace: 'Buscar estabelecimento',
      searchPlaceHint: 'Digite nome, bairro, cidade',
      poweredBy: 'Endereços © OpenStreetMap',
      itemsTitle: 'Itens',
      searchItems: 'Buscar item…',
      noItems: 'Nenhum item ainda. Adicione o primeiro.',
      newItem: 'Novo item',
      editItem: 'Editar item',
      itemName: 'Nome do item',
      category: 'Categoria',
      noCategory: 'Sem categoria',
      newCategory: 'Nova categoria',
      categoryHint: 'ex.: Grãos, Limpeza, Bebidas',
      notes: 'Observações',
      notesHint: 'ex.: sem lactose, comprar maduro',
      unit: 'Unidade',
      minStock: 'Estoque mínimo',
      minStockHint: 'Avisa quando o estoque fica abaixo disso',
      photo: 'Foto',
      addPhoto: 'Adicionar foto',
      changePhoto: 'Trocar foto',
      removePhoto: 'Remover foto',
      barcodes: 'Códigos de barras',
      noBarcodes: 'Nenhum código ainda.',
      addBarcode: 'Adicionar código',
      scan: 'Escanear',
      deleteItem: 'Excluir item',
      deleteConfirm: 'Excluir este item?',
      limitReached: 'Limite de itens do plano grátis atingido. Assine o Pro para itens ilimitados.',
      duplicateBarcode: 'Esse código já está cadastrado em outro item.',
      storesTitle: 'Lojas',
      noStores: 'Nenhuma loja ainda. Adicione a primeira.',
      newStore: 'Nova loja',
      editStore: 'Editar loja',
      storeName: 'Nome da loja',
      city: 'Cidade',
      neighborhood: 'Bairro',
      deleteStore: 'Excluir loja',
      units: { un: 'unidade', kg: 'kg', g: 'g', l: 'litro', ml: 'ml' },
    },
    scanner: {
      title: 'Escanear código',
      manualLabel: 'Ou digite o código',
      manualPlaceholder: 'Número do código de barras',
      cameraError: 'Câmera indisponível. Digite o código manualmente.',
      torch: 'Lanterna',
    },
    errors: {
      not_authenticated: 'Você precisa entrar para continuar',
      already_in_household: 'Você já faz parte de uma casa',
      no_household: 'Você ainda não tem uma casa',
      invalid_invite: 'Convite inválido ou expirado',
      rate_limited: 'Muitas tentativas, aguarde um instante',
      item_limit_reached: 'Limite de itens do plano grátis atingido.',
      barcode_exists: 'Esse código de barras já existe.',
      read_only: 'Você é visitante (somente leitura).',
      forbidden: 'Você não tem permissão para isso.',
      email_failed: 'Não foi possível enviar o e-mail agora. Tente de novo.',
      invalid_token: 'Link inválido ou expirado.',
      invite_email_mismatch: 'Este convite é para outro e-mail.',
      disposable_email: 'Use um e-mail permanente — endereços descartáveis não são aceitos.',
      pwned_password: 'Essa senha apareceu em vazamentos de dados. Escolha outra.',
      account_locked: 'Muitas tentativas. Espere alguns minutos e tente de novo.',
      captcha_failed: 'Não foi possível confirmar que você não é um robô. Recarregue e tente de novo.',
      email_suppressed: 'Esse e-mail não pode receber mensagens (retornou erro antes).',
      email_not_verified: 'Verifique seu e-mail antes de convidar alguém.',
      generic: 'Algo deu errado, tente de novo',
      list_limit_reached: 'Limite de listas do plano grátis atingido.',
      member_limit_reached: 'Limite de membros do plano grátis atingido.',
      pro_required: 'Esse recurso é exclusivo do plano Pro.',
      already_subscribed: 'Esta casa já tem uma assinatura ativa.',
      provider_unavailable: 'Assinatura indisponível no momento.',
      provider_error: 'Não foi possível processar a assinatura agora. Tente de novo.',
      coupon_invalid: 'Cupom inválido.',
      coupon_exhausted: 'Este cupom esgotou.',
      coupon_expired: 'Este cupom expirou.',
      coupon_already_redeemed: 'Esta casa já resgatou este cupom.',
      uf_unsupported: 'Importação ainda não disponível neste estado.',
      state_unsupported: 'Importação ainda não disponível neste estado (em breve).',
      nfce_invalid_qr: 'Esse código não é de uma nota fiscal.',
      nfce_invalid_key: 'Não foi possível reconhecer essa nota fiscal.',
      nfce_parse_failed: 'Não consegui ler essa nota. Tente de novo mais tarde.',
      nfce_portal_error: 'Portal da nota fiscal indisponível. Tente mais tarde.',
      nfce_provider_error: 'Não foi possível consultar a nota agora. Tente de novo.',
      nfce_timeout: 'A consulta demorou mais que o esperado. Tente de novo.',
      nfce_quota_free: 'Limite de notas do plano grátis atingido.',
      nfce_quota_pro: 'Limite mensal de importações atingido.',
      ai_unavailable: 'Geração por texto indisponível no momento.',
      ai_generation_failed: 'Não consegui gerar agora, tente de novo.',
      prompt_too_short: 'Descreva um pouco mais o que você precisa.',
      prompt_too_long: 'Texto muito longo — resuma em até 500 caracteres.',
    },
    billing: {
      plan: 'Plano',
      freeName: 'Grátis',
      proName: 'Pro',
      proPitch: 'Itens ilimitados, histórico de preços completo, membros ilimitados.',
      upgrade: 'Seja Pro',
      comingSoon: 'Em breve — assinatura via Pix ou cartão.',
      current: 'Seu plano: {{plan}}',
      hiddenBannerTitle: '{{items}} itens e {{lists}} listas ocultos',
      hiddenBannerCta: 'O Pro revela tudo — toque para assinar',
      monthly: 'Mensal',
      yearly: 'Anual',
      perMonth: '/mês',
      perYear: '/ano',
      cpfLabel: 'CPF ou CNPJ',
      cpfHint: 'Necessário para emitir a cobrança (Pix ou cartão).',
      subscribeMonthly: 'Assinar mensal',
      subscribeYearly: 'Assinar anual',
      unavailable: 'Assinatura indisponível no momento.',
      statusActive: 'Ativo',
      statusOverdue: 'Pagamento atrasado',
      statusPending: 'Aguardando pagamento',
      statusCanceled: 'Cancelado',
      cycleLabel: 'Ciclo: {{cycle}}',
      nextDueDate: 'Próxima cobrança: {{date}}',
      cancelSubscription: 'Cancelar assinatura',
      cancelConfirmTitle: 'Cancelar assinatura Pro?',
      cancelConfirmMessage: 'Você continua Pro até o fim do período pago. Depois disso, a casa volta ao plano Grátis.',
      cancelConfirmCta: 'Cancelar assinatura',
      couponLabel: 'Tenho um cupom',
      couponPlaceholder: 'Código do cupom',
      couponRedeem: 'Resgatar cupom',
      couponSuccess: 'Pro até {{date}}',
      proOverride: 'Pro concedido — sem cobrança',
      proPitchFull: 'Assine o Pro e desbloqueie:',
      benefitItems: 'Itens ilimitados',
      benefitLists: 'Listas ilimitadas',
      benefitMembers: 'Membros ilimitados',
      benefitHistory: 'Histórico de preços completo',
      benefitPhotos: 'Fotos nos itens e recibos',
      benefitAnalytics: 'Análise de gastos completa',
      benefitExport: 'Exportar preços em CSV',
      paywallTitle: 'Recurso Pro',
      paywallCta: 'Seja Pro',
      photoPaywallPitch: 'Fotos de itens e recibos são exclusivas do plano Pro.',
      analyticsPaywallPitch: 'Análise de gastos completa é exclusiva do plano Pro.',
      exportPaywallPitch: 'Exportar preços em CSV é exclusivo do plano Pro.',
      nfcePaywallPitch: 'Você atingiu o limite de notas fiscais importadas este mês no plano Grátis.',
      nlListPaywallPitch: 'Gerar listas por texto é um recurso exclusivo do plano Pro.',
      forecastPaywallPitch: 'Saiba quando cada item vai acabar — a previsão de reposição é exclusiva do plano Pro.',
      priceInsightsPaywallPitch: 'Descubra se é hora de comprar e qual marca é mais barata — os insights de preço são exclusivos do plano Pro.',
    },
    nfce: {
      importButton: 'Importar nota (QR)',
      scanAgain: 'Escanear nota',
      reviewTitle: 'Revisar nota fiscal',
      loading: 'Consultando nota…',
      processing: 'Consultando a nota no portal… pode levar até 1 min.',
      alreadyImported: 'Essa nota já foi importada.',
      itemsFound: '{{count}} item encontrado',
      itemsFound_other: '{{count}} itens encontrados',
      matched: 'Encontrado no seu catálogo',
      newItem: 'Novo item',
      ignore: 'Ignorar',
      ignored: 'Ignorado',
      changeMatch: 'Trocar',
      createInline: 'Criar item',
      storeStepTitle: 'Loja da nota',
      storeMatchedByCnpj: 'Loja reconhecida pelo CNPJ',
      storeCreateNew: 'Cadastrar como loja nova',
      confirm: 'Confirmar importação',
      confirming: 'Importando…',
      confirmed: 'Nota importada com sucesso.',
      qty: 'Qtd.',
      unitPrice: 'Preço unitário',
    },
    nlList: {
      textFieldLabel: 'Descreva o que precisa (opcional)',
      textFieldPlaceholder: 'Ex.: churrasco pra 10 pessoas',
      addByText: 'Adicionar por texto',
      reviewTitle: 'Revisar lista gerada',
      generating: 'Gerando itens…',
      noItemsWarning: 'Não entendi itens nesse texto. Tente descrever de outro jeito.',
      itemsFound: '{{count}} item gerado',
      itemsFound_other: '{{count}} itens gerados',
      qty: 'Qtd.',
      confirm: 'Adicionar à lista',
      confirming: 'Adicionando…',
    },
    settings: {
      privacy: 'Política de privacidade',
      title: 'Ajustes',
      account: 'Conta',
      data: 'Seus dados',
      syncNow: 'Sincronizar agora',
      syncHint: 'Envia e baixa mudanças manualmente.',
      exportData: 'Exportar meus dados',
      exportHint: 'Baixa tudo em JSON (LGPD).',
      exportCsv: 'Exportar preços (CSV)',
      exportCsvHint: 'Histórico de preços em planilha.',
      restore: 'Restaurar backup',
      restoreHint: 'Importa um JSON exportado neste dispositivo.',
      restoreConfirm: 'Importar este backup? Os dados serão mesclados no dispositivo.',
      deleteAccount: 'Excluir conta e dados',
      deleteHint: 'Apaga sua conta e toda a casa. Irreversível.',
      deleteConfirm: 'Tem certeza? Isso apaga TUDO e não pode ser desfeito.',
      seedItems: 'Adicionar itens comuns',
      seedHint: 'Começa com itens de mercado comuns.',
    },
    shopping: {
      share: 'Compartilhar recibo',
      receiptTag: 'compra do mês',
      savedLabel: 'Você economizou',
      overLabel: 'Acima do estimado',
      vsEstimated: 'vs. {{amount}} estimado',
      receiptTotal: 'TOTAL',
      receiptSavings: 'ECONOMIA',
      toHome: 'Início',
      start: 'Iniciar compra',
      title: 'Modo compra',
      current: 'No carrinho',
      estimated: 'Estimado',
      above: 'acima',
      below: 'abaixo',
      scanToCheck: 'Escanear pra marcar',
      markBought: 'Marcar comprado',
      actualPrice: 'Preço pago',
      priceOptional: 'opcional',
      noPriceYet: 'sem preço',
      itemsWithoutPrice: '{{count}} item sem preço — toque pra preencher ou importe a nota',
      itemsWithoutPrice_other: '{{count}} itens sem preço — toque pra preencher ou importe a nota',
      actualQty: 'Quantidade',
      priceUpWarn: 'Subiu desde a última compra',
      cheaperElsewhere: 'Tem mais barato em {{store}}',
      activeStore: 'Loja da compra',
      selectActiveStore: 'Escolha a loja desta compra',
      bought: 'COMPRADO',
      boughtSection: 'Comprados',
      allBought: 'Tudo pego!',
      finish: 'Finalizar compra',
      summary: 'Resumo da compra',
      savedVsEstimate: 'Você economizou {{amount}}',
      overEstimate: 'Acima do estimado em {{amount}}',
      itemsBought: '{{count}} itens',
      noActiveList: 'Escolha uma lista pra começar',
      emptySession: 'Sem itens necessários nesta lista.',
      budget: 'Orçamento',
      attachReceipt: 'Anexar recibo',
      hideBought: 'Ocultar comprados',
      showBought: 'Mostrar comprados',
      quickAdd: 'Adicionar item',
      confirm: 'Confirmar',
      back: 'Voltar',
    },
    sync: {
      offline: 'Offline',
      pending_one: '{{count}} pendente',
      pending_other: '{{count}} pendentes',
      syncing: 'Sincronizando…',
      synced: 'Sincronizado ✓',
      error: 'Erro ao sincronizar',
      stuck_one: '{{count}} travado · toque p/ reenviar',
      stuck_other: '{{count}} travados · toque p/ reenviar',
    },
    categories: {
      title: 'Categorias',
      empty: 'Nenhuma categoria ainda.',
      newPlaceholder: 'Nova categoria',
      delete: 'Excluir categoria',
      deleteConfirm: 'Excluir a categoria "{{name}}"? Os itens ficam sem categoria.',
      hide: 'Ocultar',
    },
    inventory: {
      title: 'Inventário',
      count: 'Contagem',
      consume: 'Consumo',
      adjust: 'Ajuste',
      used: 'Quantidade usada',
      reason: 'Motivo',
      history: 'Movimentos',
      noMovements: 'Sem movimentos.',
      filter: { all: 'Todos', low: 'Acabando', out: 'Zerado' },
      status: { low: 'Acabando', out: 'Zerado' },
      move: { purchase: 'Compra', consumption: 'Consumo', adjustment: 'Ajuste', count: 'Contagem' },
    },
    prefs: {
      sortBy: 'Ordenar:',
      byName: 'Nome',
      byCategory: 'Categoria',
      compact: 'Compacto',
      comfortable: 'Confortável',
    },
    analytics: {
      title: 'Análise de gastos',
      empty: 'Sem compras finalizadas ainda.',
      spendByMonth: 'Gasto por mês',
      spendByCategory: 'Gasto por categoria',
      topItems: 'Itens mais caros',
      total: 'Total',
      print: 'Imprimir / PDF',
    },
    review: {
      title: 'Revisar compra',
      start: 'Começar compra',
      empty: 'Nada a comprar — estoque em dia.',
      showInStock: 'Mostrar {{count}} em estoque',
      hideInStock: 'Ocultar em estoque',
    },
    history: {
      title: 'Histórico de compras',
      empty: 'Nenhuma compra finalizada ainda.',
      accumulatedSavings: 'Economia acumulada',
    },
    members: {
      title: 'Membros',
      you: 'você',
      remove: 'Remover membro',
      removeConfirm: 'Remover {{name}} da casa?',
      roles: { owner: 'Dono', admin: 'Admin', member: 'Membro', viewer: 'Visitante' },
    },
    activity: {
      title: 'Atividades',
      empty: 'Nenhuma atividade ainda.',
      action: { item_added: 'adicionou um item', list_created: 'criou uma lista', shopping_completed: 'finalizou uma compra', member_removed: 'removeu um membro' },
    },
    comments: {
      title: 'Comentários',
      placeholder: 'Escreva um comentário',
      send: 'Enviar',
    },
    search: {
      placeholder: 'Buscar item, marca, categoria…',
      allCategories: 'Todas categorias',
      saveFilter: 'Salvar filtro',
      status: { all: 'Todos', instock: 'Em estoque', low: 'Acabando', out: 'Zerado' },
    },
    priceScan: {
      scan: 'Escanear preço',
      title: 'Ler preço',
      capture: 'Capturar',
      reading: 'Lendo…',
      notFound: 'Não consegui ler. Aproxime e tente de novo.',
      cameraError: 'Câmera indisponível.',
      hint: 'Enquadre o valor na etiqueta e capture.',
    },
    onboarding: {
      skip: 'Pular',
      next: 'Próximo',
      start: 'Começar',
      seedItems: 'Adicionar itens comuns do dia a dia',
      s1Title: 'Suas listas, do seu jeito',
      s1Body: 'Crie listas do mês ou avulsas (churrasco, festa). O app calcula o que falta comprar.',
      s2Title: 'Preço certo, loja certa',
      s2Body: 'Registre preços e veja onde está mais barato. O app avisa quando um preço sobe.',
      s3Title: 'Compre offline, sem stress',
      s3Body: 'No mercado, escaneie e marque. Veja o total em tempo real — funciona sem internet.',
    },
    common: {
      confirm: 'Confirmar',
      loading: 'Carregando…',
      cancel: 'Cancelar',
      add: 'Adicionar',
      save: 'Salvar',
      saving: 'Salvando…',
      delete: 'Excluir',
      edit: 'Editar',
      back: 'Voltar',
    },
  },
};
